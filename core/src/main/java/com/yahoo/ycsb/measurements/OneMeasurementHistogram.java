...
public class OneMeasurementHistogram extends OneMeasurement {
    ...
    private void exportGeneralMeasurements(MeasurementsExporter exporter) throws IOException {
        ...
        exporter.write(getName(), "AverageLatency(ms)", usToMs("%.2f", (((double) totallatency.get()) / ((double) operations.get())) ));
        exporter.write(getName(), "MinLatency(ms)", usToMs("%.2f", min.get() ));
        exporter.write(getName(), "MaxLatency(ms)", usToMs("%.2f", max.get() ));
        ...
        for (int i = 0; i < _buckets.get(); i++) {
            ...
            if (((double) opcounter) / ((double) operations.get()) >= 0.9999) {
                exporter.write(getName(), "99.99thPercentileLatency(ms)", i);
                break;
            }
        }
    ...
    public String getSummary() {
        ...
        return "[" + getName() + " AverageLatency(ms)=" + usToMs("%.2f", report) + "]";
    }
